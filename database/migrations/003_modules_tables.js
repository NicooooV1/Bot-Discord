...
exports.up = async function (knex) {
  ...
  // === Événements serveur ===
  if (!(await knex.schema.hasTable('server_events'))) {
  ...
  // === Inventaire RP ===
  if (!(await knex.schema.hasTable('rp_inventory'))) {
    await knex.schema.createTable('rp_inventory', (t) => {
      ...
      t.foreign('character_id').references('id').inTable('rp_characters').onDelete('CASCADE');
    });
  }
